...
struct TemporaryConnection
{
    ...
    TemporaryConnection(
        size_t conn, redisContext* c, redisReply* r)
...
struct ConnectionConfiguration
{
    ...
    ConnectionConfiguration(
        std::string d_host = "localhost", std::string d_auth = EMPTY,
        size_t d_database_number = 0, uint16_t d_port = 6379)
    {
        host = d_host;
        auth = d_auth;
    ...
    ConnectionConfiguration(
        const ConnectionConfiguration& copy)
    {
        host = copy.host;
        auth = copy.auth;
...
class ConnectionManager
{
    ...
    size_t new_request()
    {
        std::map<size_t, TemporaryConnection*>::iterator iter =
            __connections.begin();
        for (size_t n = 0; n < num_requests, iter != __connections.end();
            ++n, ++iter) {
...
public:
    ConnectionManager(
        const ConnectionConfiguration& defaultConnection)
        ...
        std::string host, int port, std::string auth = EMPTY, size_t dbnum = 0)
    {
        ...
        TemporaryConnection* conn = new TemporaryConnection(request, context,
            reply);
    ...
    {
        return requestConnection(config.host, config.port, config.auth,
