...
public:
    enum Keyspaces
    {
        none, widgets, policies, sessions, data, global_events, templates, meta
    };
    std::map<Keyspaces, std::string> __prefixes;

    AccessLayer(
        TemporaryConnection& conn, Keyspaces keyspace = none)
        : connection(conn)
    {
        __keyspace = keyspace;
...
static std::map<AccessLayer::Keyspaces, std::string> keyspacePrefixes =
    create_map<AccessLayer::Keyspaces, std::string>(AccessLayer::widgets,
        "widgets")(AccessLayer::policies, "policies")(AccessLayer::sessions,
        "s")(AccessLayer::data, "data")(AccessLayer::global_events, "events")(
        AccessLayer::templates, "templates")(AccessLayer::meta, "meta");
