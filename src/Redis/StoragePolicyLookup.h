...
public:
	...
	StoragePolicyLookup (const std::string& policyName);

	inline std::string buildPolicyNode(const std::string& policyName) const
	{ return "policies."+policyName;}

	const uint16_t getProperties() { return __flags; }

	inline bool isAppendMode() { return __flags & set_append;}
	inline bool policyExists() { return __flags & exists; }
	inline bool isEncrypted() { return __flags & encrypted; }
