...
class WidgetPropertyAddress
{
    ...
    enum Result
    {
        DOES_NOT_EXIST = 0x0, EXISTS_AT_NODE = 0x1, EXISTS_AT_TPL = 0x2
    };
    Mogu* __app;

public:
    WidgetPropertyAddress(
        std::string base, const char* field)
    {
    ...
    inline bool exists_at(
        std::string prefix, std::string base, const char* field)
    {
    ...
    inline bool exists_at_node(
        std::string base, const char* field)
    {
        return exists_at("widgets", base, field);
    }

    inline bool exists_at_template(
        std::string base, const char* field)
    {
...
inline bool setHasMember(
    const std::string& set, const char* member)
{
    mApp;
...
inline bool widgetHasProperty(
    const std::string& node, const char* prop)
{
    WidgetPropertyAddress lookup(node, prop);
    return (int) lookup.result > 0;
}

inline bool widgetHasBooleanProperty(
    const std::string& node, const char* prop)
{
    mApp;
...
inline std::string getHashEntry(
    std::string& hash_node, const char* hash_field, std::string alt = EMPTY)
{
    mApp;
...
inline std::string getHashEntry(
    std::string& hash_node, int list_num, const char* hash_field,
    std::string alt = EMPTY)
{
    mApp;
    const char* chashnode = hash_node.c_str();
    app->redisExec(Mogu::Keep, "hexists %s.%d %s", chashnode, list_num,
        hash_field);
    if (!redisReply_TRUE) return alt;
    app->redisExec(Mogu::Keep, "hget %s.%d %s", chashnode, list_num,
        hash_field);
 ...
 */
inline WidgetType getWidgetType(
    std::string nodeName)
{
    mApp;
    WidgetType __type;
    NodeValue val;
    std::string reply_str = getWidgetProperty(nodeName, "type");
