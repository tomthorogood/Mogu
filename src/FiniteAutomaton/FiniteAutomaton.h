...
public:
     ...
     */
    State(
        FiniteAutomaton* automaton, uint8_t state_type, const E& state_name,
        const V& validOutputs = 0, T (*parse_callback)(
            const U&)=0)
        : AbstractState(automaton), __valid_outputs(validOutputs), __name(
            state_name)
    {
     ...
     */
    inline AbstractState* destination(
        T type)
    {
     ...
     */
    inline State<T, U, E, V>& registerDestination(
        T type, AbstractState* other)
    {
