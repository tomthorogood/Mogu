...
bool LoginAuthenticator::user_exists()
{
    ...
    app->redisExec(Mogu::Keep, "hexists %s %s", __NODE_USERID_LOOKUP,
        cenc_login);
    if (redisReply_TRUE) {
        /* If the user login exists, get the internal user id */
        app->redisExec(Mogu::Keep, "hget %s %s", __NODE_USERID_LOOKUP,
            cenc_login);
...
bool LoginAuthenticator::user_salted()
{
    ...
    app->redisCommand("hexists %s %s", __NODE_SALT_LOOKUP, cenc_id);
    if (redisReply_TRUE) {
...
{
    mApp;
    raw_auth_string = Security::create_raw_auth_string(plain_userlogin, salt,
        plain_passwd);
    prf_auth_string = raw_auth_string;
    app->redisExec(Mogu::Keep, "hget %s %s", __NODE_COLLISION_STR_LOOKUP,
        mogu_userid.c_str());
    ...
    app->redisCommand("hexists %s %s", __NODE_AUTH_LOOKUP, cprauth_str);
    if (redisReply_TRUE) {
        app->redisCommand("hget %s %s", __NODE_AUTH_LOOKUP, cprauth_str);
...
bool LoginAuthenticator::authtoken_matches()
{
    ...
    app->redisCommand("hexists %s %s", csession, __AUTH_HASH);
    if (!redisReply_TRUE) return false;

    app->redisCommand("hget %s %s", csession, __AUTH_HASH);
