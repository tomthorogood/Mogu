        ...
        std::vector <std::string>& results)
{
    if (group_id != 0 && results.size() == 0) {
        Redis::MoguQueryHandler& db = manager.getContext(Prefix::group);
...
int GroupController::consumeGroupId(Redis::MoguQueryHandler& db)
{
        ...
        bool userIsAdmin=false)
{
    ...
    Redis::MoguQueryHandler& usr = manager.getContext(Prefix::user);
...
void GroupController::destroyGroup() {
    ...
    Redis::MoguQueryHandler& db = manager.getContext(Prefix::user);
...
void GroupController::joinGroup()
{
    ...
    Redis::MoguQueryHandler& gdb = manager.getContext(Prefix::group);
    Redis::MoguQueryHandler& udb = manager.getContext(Prefix::user);
