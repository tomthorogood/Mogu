...
Moldable::Moldable(
    ...
    : Wt::WContainerWidget(), mchildren(), __style_changed(this), __index_changed(
        this), __failed_test(this), __succeeded_test(this), __hidden_changed(
        this), __onload(this)
{
    __tmpl = tpl;
#ifdef DEBUG
    ...
    Enums::SignalActions::SignalAction action)
{
    ...
    NodeValue v;
    while (b != EMPTY) {
        app->interpreter().giveInput(b, v, this,
            &Parsers::enum_callback<Parsers::SignalActionParser>);
        Enums::SignalActions::SignalAction block =
            (Enums::SignalActions::SignalAction) v.getInt();
