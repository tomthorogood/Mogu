    ...
    const std::string& node)
{
    ...
    if (widgetHasProperty(node, "location")) __tmpl->location =
        getWidgetProperty(node, "location");

    return __tmpl;
}

MoldableTemplate* conceptualize(
    const MoldableTemplate* __orig, size_t index)
{
    mApp;
    ...
    MoldableTemplate* __tmpl, Moldable* m)
{
    ...
    if (__tmpl->flags & is_named) {
        std::string name =
            (__tmpl->name != EMPTY) ?
                __tmpl->name : getWidgetProperty(__tmpl->node, "name");
