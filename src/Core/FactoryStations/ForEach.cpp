...
AbstractForEach::AbstractForEach(MoldableTemplate& tmpl, Moldable& parent)
:
	__values()
	,__tmpl(tmpl)
{
...
void AbstractForEach::execute(Moldable& parent)
{
	using namespace Parsers::StyleParser;

	getNumChildren();
		populateValues();
		mapped inner_type = (mapped)
			getWidgetType(
...
ForEachData::ForEachData(MoldableTemplate& tmpl, Moldable& parent)
...
ForEachGroupMember::ForEachGroupMember(MoldableTemplate& tmpl, Moldable& parent)
:
		AbstractForEach(tmpl,parent)
		, memberSessions()
		, groupName(app->getGroup())
		, groupManager(groupName)
{
	execute(parent);
}
...
void ForEachGroupMember::getNumChildren()
{
	...
	std::vector <std::string>& accessible =
			groupManager.getMembership(groupManager.getMemberRank(mogu_id), 0);
	__tmpl.num_children = accessible.size();
	for (size_t i = 0; i < __tmpl.num_children; ++i)
	{
		app->redisExec(Mogu::Keep, "hget %s %s",
				__NODE_SESSION_LOOKUP, accessible[i].c_str());
